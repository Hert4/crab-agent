...
(function() {
   ...
   * @param {boolean} viewportOnly - Only include viewport-visible elements
   * @returns {{ lines: string[], nodeCount: number, truncated: boolean }}
   */
  window.__generateAccessibilityTree = function(filter, maxDepth, focusRefId, viewportOnly) {
    filter = filter || 'all';
    ...
    function walk(node, depth) {
      ...
      const include = shouldInclude(node, filter, viewportOnly && !focusRefId);
    ...
    return { lines, nodeCount, truncated };
  };

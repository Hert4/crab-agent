...
(function() {
   ...
   * @param {number} maxChars - Max output characters (default 50000)
   * @returns {{ success: boolean, lines: string[], nodeCount: number, truncated: boolean, error?: string }}
   */
  function generateAccessibilityTree(filter = 'all', depth = 15, refId = null, includeCoords = true, maxChars = 50000) {
    try {
      ...
      let charCount = 0;
      let truncated = false;
      const maxDepth = Math.min(20, Math.max(1, depth));

...
(function() {
  ...
  const STABLE_INDEX_STATE_KEY = '__agentSStableIndexState';

  function getPageStableState() {
    const pageKey = `${window.location.origin}${window.location.pathname}`;
    const existing = window[STABLE_INDEX_STATE_KEY];

    if (!existing || existing.pageKey !== pageKey || typeof existing.nextIndex !== 'number' || !existing.signatureToIndex) {
  ...
  function buildStableSignature(tagName, attributes, xpath) {
    const normalize = (value, max = 120) => String(value || '').trim().toLowerCase().slice(0, max);
    return [
      normalize(tagName, 40),
      normalize(attributes?.id, 80),
      normalize(attributes?.name, 80),
      normalize(attributes?.role, 80),
      normalize(attributes?.['aria-label'], 120),
      normalize(attributes?.placeholder, 120),
      normalize(xpath, 220)
    ].join('|');
  }
  ...
  function isInteractiveElement(element) {
    ...
    if (style.cursor === 'pointer') {
      ...
      if (hasExplicitHint) return true;

      // For Vue/React apps: cursor:pointer + meaningful text/class suggests clickable
      const text = (element.innerText || element.textContent || '').trim();
      const className = element.className || '';
      const hasClickableClass = /btn|button|click|action|link|add|new|create|edit|delete|remove|save|cancel|submit|open|close|toggle|menu|nav|tab/i.test(className);
      const hasMeaningfulText = text.length > 0 && text.length < 100;

      // Accept if has clickable class pattern OR has short meaningful text
      if (hasClickableClass || hasMeaningfulText) return true;
    }
    ...
    // Vue/React framework detection - these frameworks attach event listeners programmatically
    // Vue 2: __vue__, Vue 3: __vueParentComponent, React: __reactFiber$*, __reactProps$*
    const hasVueInstance = element.__vue__ || element.__vueParentComponent;
    const hasReactInstance = Object.keys(element).some(key =>
      key.startsWith('__reactFiber$') || key.startsWith('__reactProps$') || key.startsWith('__reactEvents$')
    );

    if (hasVueInstance || hasReactInstance) {
      // Framework-managed element - check if it looks clickable
      const style = window.getComputedStyle(element);
      const text = (element.innerText || element.textContent || '').trim();
      const looksClickable =
        style.cursor === 'pointer' ||
        (text.length > 0 && text.length < 80) ||
        element.hasAttribute('disabled') || // has disabled attr means it's meant to be interactive
        /btn|button|link|action/i.test(element.className || '');
  ...
  function getElementText(element, maxLength = 100) {
    ...
    // If element has no text, try to get context from nearby elements
    // This helps with checkboxes, icons, and other elements without text
    if (!text) {
      // Check aria-label first
      text = element.getAttribute('aria-label') || '';

      // Try to find text from parent row/container (for checkboxes in lists)
      if (!text) {
        const parent = element.closest('[class*="item"], [class*="row"], [class*="list"], [role="listitem"], [role="option"]');
        if (parent && parent !== element) {
          const parentText = (parent.innerText || parent.textContent || '').trim().replace(/\s+/g, ' ');
          if (parentText && parentText.length < 150) {
            text = `[in: ${parentText.substring(0, 80)}]`;
          }
        }
      }

      // Try next sibling text (for checkbox + label pattern)
      if (!text && element.nextElementSibling) {
        const siblingText = (element.nextElementSibling.innerText || element.nextElementSibling.textContent || '').trim();
        if (siblingText && siblingText.length < 100) {
          text = `[near: ${siblingText.substring(0, 50)}]`;
        }
      }
    }
  ...
  function buildDomTree(options = {}) {
    ...
    const elementMap = {};
    let collected = 0;
    const stableState = getPageStableState();
    ...
    for (const element of allElements) {
      ...
      // Skip menu/list CONTAINERS that have multiple menu items as children
      // This prevents indexing the container instead of individual items
      const elementRole = element.getAttribute('role');
      const isContainerRole = ['menu', 'menubar', 'listbox', 'list', 'tablist', 'tree'].includes(elementRole);
      const menuItemChildren = element.querySelectorAll(':scope > [role="menuitem"], :scope > [role="option"], :scope > li');
      if (isContainerRole && menuItemChildren.length > 1) {
        continue;
      }
      // Also skip any element (regardless of role) that directly contains 2+ menuitems
      const directMenuItems = element.querySelectorAll(':scope > [role="menuitem"], :scope > [role="option"]');
      if (directMenuItems.length > 1) {
        continue;
      }
      // Skip elements that contain ANY menuitem descendants (they're containers)
      const anyMenuItems = element.querySelectorAll('[role="menuitem"], [role="option"]');
      if (anyMenuItems.length > 1) {
        continue;
      }
      // Skip containers with multiple <li> children (common menu pattern)
      const liChildren = element.querySelectorAll(':scope > li, :scope > ul > li');
      if (liChildren.length > 1) {
        continue;
      }
      // Skip JupyterLab/Lumino menu containers (lm-Menu, jp-Menu patterns)
      const className = element.className || '';
      const isLuminoMenuContainer = /\b(lm-Menu|jp-Menu|lm-MenuBar|jp-MenuBar)\b/.test(className) &&
                                     !/(lm-Menu-item|jp-Menu-item)/.test(className);
      if (isLuminoMenuContainer) {
        const menuItems = element.querySelectorAll('[class*="Menu-item"], li');
        if (menuItems.length > 1) {
          continue;
        }
      }
      // Skip any element with multiple clickable list items inside
      const clickableItems = element.querySelectorAll('li[class*="item"], li[class*="Item"], [class*="menu-item"], [class*="Menu-item"], [class*="MenuItem"]');
      ...
      const signature = buildStableSignature(tagName, attributes, xpath);
      let stableIndex = stableState.signatureToIndex[signature];
      if (stableIndex === undefined) {
        stableIndex = stableState.nextIndex++;
        stableState.signatureToIndex[signature] = stableIndex;
      }
      ...
      elementMap[stableIndex] = element;

      // Create highlight
      if (highlightElements) {
        const color = HIGHLIGHT_COLORS[stableIndex % HIGHLIGHT_COLORS.length];
        createHighlight(element, stableIndex, color);
      }
    ...
    // Group menuitems by parent menu, sort by y-position, assign ordinals
    const menuItemOrdinals = {};
    const menuItems = elements.filter(el => el.attributes?.role === 'menuitem' || el.attributes?.role === 'option');
    if (menuItems.length > 0) {
      // Group by parent menu (use x-coordinate range as proxy for same menu column)
      const menuGroups = {};
      for (const el of menuItems) {
        // Use x-coordinate bucket (items in same vertical menu have similar x)
        const xBucket = Math.round((el.rect?.x || 0) / 50) * 50;
        if (!menuGroups[xBucket]) menuGroups[xBucket] = [];
        menuGroups[xBucket].push(el);
      }
      // Sort each group by y-coordinate and assign ordinals
      for (const group of Object.values(menuGroups)) {
        group.sort((a, b) => (a.rect?.y || 0) - (b.rect?.y || 0));
        group.forEach((el, idx) => {
          menuItemOrdinals[el.index] = { pos: idx + 1, total: group.length };
        });
      }
    }
    ...
    for (const el of elements) {
      ...
      // Mark contenteditable elements clearly as INPUT targets
      const actualElement = elementMap[el.index];
      if (actualElement?.isContentEditable) {
        line += ` [EDITABLE INPUT]`;
      }
      ...
      // Add ordinal for menu items (e.g., [item 3/7])
      const ordinal = menuItemOrdinals[el.index];
      if (ordinal) {
      ...
      // Add position hint for ALL elements to help model distinguish elements with same text
      if (el.rect) {
        line += ` @(${el.rect.x + Math.round(el.rect.width/2)},${el.rect.y + Math.round(el.rect.height/2)})`;
      }
